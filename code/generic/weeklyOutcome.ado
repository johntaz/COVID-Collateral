...
qui merge m:1 patid using "$dataDir\cr_`study'_strat_summary.dta", keep(match) nogen
}
...
noi di as text "Removing patients with outcomes < `outcomegap' apart..." 
noi di ""
...
* overall
tempname denom
qui postfile `denom' weekDate numOutcome time category str15(stratifier) using "$dataDir\cr_`study'_outcome_overall.dta", replace
...
* overall
tempname denom
qui postfile `denom' weekDate numOutcome time category str15(stratifier) using "$dataDir\cr_`study'_outcome_age.dta", replace
...
* overall
tempname denom
qui postfile `denom' weekDate numOutcome time category str15(stratifier) using "$dataDir\cr_`study'_outcome_gender.dta", replace
	forvalues g = 1/2 {
		forvalues i = 1/`numWeeks' {
		cap drop outcome
		local k = `i' - 1 
...
* overall
tempname denom
qui postfile `denom' weekDate numOutcome time category str15(stratifier) using "$dataDir\cr_`study'_outcome_region.dta", replace
...
local denominators "overall age gender region"
	foreach i of local denominators {
		qui append using "$dataDir\cr_`study'_outcome_`i'.dta"
		format weekDate %td
		qui save "$dataDir\cr_`study'_weekly_outcomes.dta", replace
	}
	
	foreach i of local denominators {
  	qui erase "$dataDir\cr_`study'_outcome_`i'.dta"
}
...
noi di as result "$dataDir\cr_`study'_weekly_outcomes.dta"
