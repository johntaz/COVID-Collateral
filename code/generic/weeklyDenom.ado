...
qui merge 1:1 patid using "$dataDir\diabetesType.dta", keep(match) nogen keepusing(patid) // update to include type
}
...
qui merge 1:1 patid using "$dataDir\diabetesType.dta", keep(match) nogen
...
tempname denom
qui postfile `denom' weekDate numEligible time category str15(stratifier) using "$dataDir\cr_`study'_denom_overall.dta", replace
...
* gender
tempname denom
qui postfile `denom' weekDate numEligible time category str15(stratifier) using "$dataDir\cr_`study'_denom_age.dta", replace
...
* gender
tempname denom
qui postfile `denom' weekDate numEligible time category str15(stratifier) using "$dataDir\cr_`study'_denom_gender.dta", replace
	forvalues g = 1/2 {
		forvalues i = 1/`numWeeks' {
		cap drop eligibleFlag
		local k = `i' - 1 
...
noi di as text "Progress..."
tempname denom
qui postfile `denom' weekDate numEligible time category str15(stratifier) using "$dataDir\cr_`study'_denom_region.dta", replace
...
* Save summary
keep patid pracid gender yob region gender 
save "$dataDir\cr_`study'_strat_summary.dta", replace
*****
...
clear 
local denominators "overall age gender region"
	foreach i of local denominators {
		qui append using "$dataDir\cr_`study'_denom_`i'.dta"
		format weekDate %td
		qui save "$dataDir\cr_`study'_weekly_denoms.dta", replace
	}
	
	foreach i of local denominators {
  	qui erase "$dataDir\cr_`study'_denom_`i'.dta"
}
...
noi di as result "$dataDir\cr_`study'_strat_summary.dta"
noi di ""
noi di as result "Combined weekly denominator file: "
noi di as result "$dataDir\cr_`study'_weekly_denoms.dta"
