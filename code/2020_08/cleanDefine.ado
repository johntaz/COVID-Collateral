...
qui save "tempData`i'.dta", replace 
local i = `i' + 1
...
if `num' > 1 { 
	forvalues i = 1/`num'  {
		qui append using "tempData`i'.dta"
		qui save "cr_`keyword'_outcomes.dta", replace
	}
}
else if `num' == 1 {
...
* Heart failure only requires first Dx ever
if "`keyword'" == "hf" {
noi di ""
noi di as text "***********************************************************************" 
noi di as text "Heart failure outcome only selects first Dx - remove other events " 	
noi di as text ""
bysort patid: egen firstDx = min(eventdate) 
format firstDx %td
keep if eventdate == firstDx
drop firstDx
}
...
noi di as result "cr_`keyword'_outcomes.dta"
*
* ------------------------------------------------------------------------------
end
